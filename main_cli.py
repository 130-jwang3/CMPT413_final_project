    ...
    def find_normal_reference(self, data_id, channel, num: int = 3, fixed: bool = False) -> list:
        ...
        if os.path.exists(image_dir):
            ...
            if num > len(normal_list):
                num = len(normal_list)
            if not normal_list:
                print(f"No normal images found in {image_dir}")
                return []
            sample_normal_list = normal_list[:num] if fixed else random.sample(normal_list, num)
            return sample_normal_list
        else:
            print(f"Directory {image_dir} does not exist. Falling back to test_structure.yaml")
            normal_list = self.structure_info.get(data_id, {}).get('normal', [])
            print(f"Normal entries in test_structure.yaml for {data_id}: {normal_list}")
            if len(normal_list) == 0:
                print(f'No normal reference found in {self.structure_yaml_path} for data_id {data_id}.')
                return []
            if num > len(normal_list):
                num = len(normal_list)
            sample_normal_list = normal_list[:num] if fixed else random.sample(normal_list, num)
