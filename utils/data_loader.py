...
class UCRSegLoader(dataSegLoader):
    def __init__(self, data_path, win_size, step, mode="train", horizon=1):
        self.data_path = data_path
        self.horizon = horizon
        self.mode = mode
        ...
        # List of dataset indices to load (based on the image: 135, 136, 137, 138)
        dataset_indices = [135, 136, 137, 138]

        # Initialize lists to store train, test, and labels data
        train_data_list = []
        test_data_list = []
        test_labels_list = []

        # Load and concatenate data from all dataset indices
        for idx in dataset_indices:
            train_file = os.path.join(self.data_path, f"{idx}_train.npy")
            test_file = os.path.join(self.data_path, f"{idx}_test.npy")
            labels_file = os.path.join(self.data_path, f"{idx}_labels.npy")

            # Load the data
            train_data = np.load(train_file)
            test_data = np.load(test_file)
            labels_data = np.load(labels_file).astype(int)

            # Append to lists
            train_data_list.append(train_data)
            test_data_list.append(test_data)
            test_labels_list.append(labels_data)

        # Concatenate all the data
        self.train = np.concatenate(train_data_list, axis=0)
        self.test = np.concatenate(test_data_list, axis=0)
        self.test_labels = np.concatenate(test_labels_list, axis=0)

        # Optionally, you can add a subset of the data for faster experimentation
        index = 0  # Starting index for subset
        length = 20000  # Length of subset (adjust as needed)
        self.train = self.train[index:index + length]
        self.test = self.test[index:index + length]
        self.test_labels = self.test_labels[index:index + length]

        print(self.train.shape, self.test.shape, self.test_labels.shape)

    def __init__(self, data_path, win_size, step, mode="train", horizon=1, dataset_index=135):
        self.data_path = data_path
        self.horizon = horizon
        self.mode = mode
        self.step = step
        self.win_size = win_size
        self.dataset_index = dataset_index  # To specify which UCR dataset to load (e.g., 135, 136, etc.)

        # Define the file paths for train, test, and labels
        train_file = os.path.join(self.data_path, f"{self.dataset_index}_train.npy")
        test_file = os.path.join(self.data_path, f"{self.dataset_index}_test.npy")
        labels_file = os.path.join(self.data_path, f"{self.dataset_index}_labels.npy")

        # Load the data
        data = np.load(train_file)
        test_data = np.load(test_file)
        self.test_labels = np.load(labels_file).astype(int)

        # Optionally, you can add a subset of the data for faster experimentation
        index = 0  # Starting index for subset
        length = 20000  # Length of subset (adjust as needed)
        self.train = data[index:index + length]
        self.test = test_data[index:index + length]
        self.test_labels = self.test_labels[index:index + length]
...
def get_loader_segment(data_path, batch_size, win_size=6, step=1, mode='train', dataset='UCR', horizon=1):
